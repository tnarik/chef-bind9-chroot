...
zone \"example.com\" {
  type master;
  file \"#{vals['zones_dir']}/db.example.com\";
  allow-transfer {
    192.168.1.2;
    192.168.1.3;
  };
...
zone \"example.net\" {
  ...
  masters {
    ...
    it "creates directory /var/run/bind#{vals['data_dir']} owned by #{vals['user']} user" do
      chef_run.node.set[:bind9][:chroot_dir] = '/var/run/bind'
      chef_run.converge(described_recipe)
      expect(chef_run).to create_directory("/var/run/bind#{vals['data_dir']}").with(
        user: vals['user'],
        group: vals['group'],
        mode: 0755,
        recursive: true
    ...
    it "creates directory /var/run/bind#{vals['log_dir']} owned by #{vals['user']} user" do
      chef_run.node.set[:bind9][:chroot_dir] = '/var/run/bind'
      chef_run.converge(described_recipe)
      expect(chef_run).to create_directory("/var/run/bind#{vals['log_dir']}").with(
        user: vals['user'],
        group: vals['group'],
        mode: 0755,
        recursive: true
    ...
    it "creates directory /var/run/bind#{vals['zones_dir']} owned by #{vals['user']} user" do
      chef_run.node.set[:bind9][:chroot_dir] = '/var/run/bind'
      chef_run.converge(described_recipe)
      expect(chef_run).to create_directory("/var/run/bind#{vals['zones_dir']}").with(
        user: vals['user'],
        group: vals['group'],
        mode: 0744,
        recursive: true
    ...
    if platform == 'ubuntu'
      ...
      it '/etc/default/bind9 notifies bind9 to restart' do
        expect(chef_run.template('/etc/default/bind9')).to notify("service[#{vals['service']}]").to(:restart)
      end
    end

    it "does not create #{vals['zones_dir']}/db.example.com" do
      expect(chef_run).to_not create_template("#{vals['zones_dir']}/db.example.com").with(
    ...
    it "creates #{vals['zones_dir']}/db.example.com.erb" do
      expect(chef_run).to create_template("#{vals['zones_dir']}/db.example.com.erb").with(
        source: 'zonefile.erb',
        user: vals['user'],
        group: vals['group'],
        mode: 0644,
    ...
    it "fills #{vals['zones_dir']}/db.example.com.erb with correct content" do
      expect(chef_run).to render_file("#{vals['zones_dir']}/db.example.com.erb").with_content(zone_file)
    end

    it "notifies #{vals['zones_dir']}/db.example.com immediately" do
      expect(chef_run.template("#{vals['zones_dir']}/db.example.com.erb")).to notify("template[#{vals['zones_dir']}/db.example.com]").to(:create).immediately
    end

    it "does not create #{vals['zones_dir']}/db.example.net.erb" do
      expect(chef_run).to_not create_template("#{vals['zones_dir']}/db.example.net.erb")
    end
      end
    end
  end
