...
describe 'bind9-chroot::chroot' do
  context 'Ubuntu 12.04' do
    ...
    it 'runs ruby_block move_config_to_chroot' do
      ...
      File.stub(:symlink?).with('/etc/bind').and_return(false)
      expect(chef_run).to run_ruby_block('move_config_to_chroot')
    end

    it 'does not run ruby_block move_config_to_chroot' do
      File.stub(:symlink?).with(anything).and_call_original
      File.stub(:symlink?).with('/etc/bind').and_return(true)
      expect(chef_run).to_not run_ruby_block('move_config_to_chroot')
    end

    it 'links bind config from chroot' do
      expect(chef_run).to create_link('/etc/bind').with(
        to: "/var/run/bind/etc/bind"
      )
    end

    it "does not create directory /var/run/bind/etc/bind/zones" do
      expect(chef_run).to_not create_directory("/var/run/bind/etc/bind/zones")
    end

    it 'does not link bind zones from chroot' do
      expect(chef_run).to_not create_link('/etc/bind/zones').with(
      to: "/var/run/bind/etc/bind/zones"
      )
    end
