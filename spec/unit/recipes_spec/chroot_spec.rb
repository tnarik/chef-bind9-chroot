  ...
  platforms.each do |platform,vals|
    vals['versions'].each do |version|
      context "On #{platform} #{version}" do
        ...
        it "creates directory /var/run/bind#{vals['config_dir']}" do
          expect(chef_run).to create_directory("/var/run/bind#{vals['config_dir']}").with(
            user: 'bind',
            group: 'bind',
            mode: 0744,
            recursive: true
        ...
        it 'links bind config from chroot' do
          expect(chef_run).to create_link(vals['config_dir']).with(
            to: "/var/run/bind#{vals['config_dir']}"
          )
        end

        it "does not create directory /var/run/bind#{vals['zones_dir']}" do
          expect(chef_run).to_not create_directory("/var/run/bind#{vals['zones_dir']}")
        end

        it 'does not link bind zones from chroot' do
          expect(chef_run).to_not create_link(vals['zones_dir']).with(
          to: "/var/run/bind#{vals['zones_dir']}"
          )
        end
